...
module.exports = {
    ...
    async execute(interaction) {
        if (!(await canUseCommand(interaction, 'kick'))) {
            return interaction.reply({ embeds: [feedback.acessoNegado()], ephemeral: true });
        }
        try {
            logger.info(`[KICK] Comando executado por ${interaction.user.tag} (${interaction.user.id}) no servidor ${interaction.guild?.name || 'DM'} (${interaction.guild?.id || 'DM'})`);
            ...
            logger.error(`[KICK] Erro ao executar kick: ${error}`);
